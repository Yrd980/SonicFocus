...
class _FocusModePageState extends State<FocusModePage> {
  ...
  bool backgroundReduce = true;
  bool safeAwareness = false;
  late VoiceLine voice;
  ...
  void didChangeDependencies() {
    ...
    voice = routeVoice is VoiceLine ? routeVoice : demoVoiceLines.first;
    initialized = true;
  }
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          padding: EdgeInsets.fromLTRB(
              t.containerPadding, t.containerPadding, t.containerPadding, 120),
          child: Center(
                    ...
                    LayoutBuilder(
                      builder: (context, constraints) {
                        ...
                        final cards = [
                          ...
                          _NoiseReductionCard(strength: strength),
                        ];
                      ...
                      builder: (context, constraints) {
                        ...
                        final controls = [
                          _ToggleControl(
                            ...
                            value: voiceEnhance,
                            onTap: () =>
                                setState(() => voiceEnhance = !voiceEnhance),
                          ),
                          _ToggleControl(
                            label: 'Background Reduce',
                            icon: Icons.filter_vintage,
                            value: backgroundReduce,
                            onTap: () => setState(
                          ...
                          _ToggleControl(
                            ...
                            value: safeAwareness,
                            onTap: () =>
                                setState(() => safeAwareness = !safeAwareness),
                          ),
                        ];
                          ...
                          children: [
                            for (var i = 0; i < controls.length; i++)
                              Expanded(
                                child: Padding(
                                  padding: EdgeInsets.only(
                                      right: i == controls.length - 1 ? 0 : 12),
                                  child: controls[i],
                                ),
                              ),
                          ],
  ...
  List<int> get _sessionWaveform {
    final boost = strength.toInt();
    return const [3, 7, 5, 8, 4, 6, 7, 2, 5, 8, 6, 4, 7, 5, 3, 8, 6, 4, 7, 5]
        .map((value) => (value + boost).clamp(1, 8))
        .toList();
  }
}
...
class _NoiseReductionCard extends StatelessWidget {
  const _NoiseReductionCard({required this.strength});
  ...
  Widget build(BuildContext context) {
    final t = context.sf;
    final suppressDb = 18 + strength.toInt() * 6;
    return GlassPanel(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
